...
@login_required
def profile(request):
    profile = Profile.objects.filter(user= request.user) #user is a part of profile, using for fileter is get doesnot work with exist
...
def myfeed(request):
    comment_count_list = []
    like_count_list = []
    posts = Post.objects.filter(username= request.user).order_by('-date_posted')  #filter allows us to get all the post that belongs to user 
    for p in posts:
...
def friendsfeed(request):
    comment_count_list = []
    like_count_list = []
    friends = Profile.objects.filter(user= request.user).values('friends')  #filter allows us to get all the post that belongs to user 
    ...
    if request.method== 'POST' and request.POST.get("like"):
        ...
        print(post_to_like)
        like_already_exist = Like.objects.filter(post_id=post_to_like, username=request.user)
...
def comments(request, post_id):
    ...
    comments = Comment.objects.filter(post=post_id) #COmment is model
...
def friends(request):
    ...
    admin_profile = Profile.objects.get(user=1) #the first person created is admin and admin's id is 1
    ...
    request_sent_profiles = user_relationships.values('receiver') #getting the profile of every person that we send the request to
    ...
    #witht the admin of website (so the admin is friend with everyone)
    ...
    #this is to process all send request or get the list of receiver
    if request.method == 'POST' and request.POST.get("send_requests"):
        receivers = request.POST.getlist("send_requests") #this object on html is checkbox and name is same but different value 
    ...
    #this is to process all receive requests or get the list of sender 
    if request.method == 'POST' and request.POST.get("receive_requests"):
        senders = request.POST.getlist("receive_requests")
        for sender in senders:
            #locate and update the relationship model for the sender to status 'accepted'
